...
def create_account_info(wallet_balance_raw: dict, config: dict):
    ...
    return {
        ...
        "balanceInBank": round(config["bybitSwingAccount"]["balanceInBank"], 2),
        "accountSize": round(wallet_balance + config["bybitSwingAccount"]["balanceInBank"], 2)
    }
...
def transform_trades(trades_raw: pd.DataFrame, instruments_info: dict, config: dict, account_info: dict) -> pd.DataFrame:
    ...
    risk_per_trade = account_info["accountSize"] * \
        config["bybitSwingAccount"]["riskPerTrade"]
    used_margin_account_usd = account_info["balanceOnExchange"] * \
        config["bybitSwingAccount"]["usedMarginAccount"]
